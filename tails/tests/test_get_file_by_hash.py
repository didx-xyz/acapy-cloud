...
async def test_get_file_by_hash_success():
    ...
    with patch("tails.routers.tails.get_s3_client", return_value=mock_s3_client):
        ...
        # Check headers
        assert response.headers["Content-Disposition"] == f"attachment; filename={tails_hash}"
        assert response.headers["Content-Length"] == str(mock_head_response["ContentLength"])
